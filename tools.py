            ...
            async with httpx.AsyncClient(timeout=15) as client:
                resp = await client.post(
                    ...
                    json={
                        ...
                        "metadata": {"source": "OutboundAI", "notes": notes}
                    }
                )
                if resp.status_code in (200, 201):
                    data = resp.json()
                    uid = data.get("uid") or data.get("booking", {}).get("uid", "")
                    return f"Cal.com booking successful! UID: {uid}"
                return f"Cal.com booking failed: {resp.text}"
        except Exception as exc:
            return f"Cal.com error: {exc}"

    @llm.function_tool
    async def cancel_calcom(self, booking_uid: str) -> str:
        """
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=15) as client:
                ...
                if resp.status_code in (200, 201, 204):
                    return "Cal.com booking cancelled successfully."
                return f"Cal.com cancel failed: {resp.text}"
        except Exception as exc:
            return f"Cal.com cancel error: {exc}"
