...
async def api_get_agent_profile_endpoint(profile_id: str):
...
async def api_update_agent_profile_endpoint(profile_id: str, req: AgentProfileRequest):
...
async def api_delete_agent_profile_endpoint(profile_id: str):
...
async def api_set_default_profile_endpoint(profile_id: str):
    try:
...
async def _dispatch_one(lk, lk_api_module, contact: dict, room_name: str,
    ...
    try:
        ...
        await lk.agent_dispatch.create_dispatch(
            lk_api_module.CreateAgentDispatchRequest(agent_name="outbound-caller", room=room_name, metadata=json.dumps(metadata))
        )
        return True
    except Exception as exc:
...
async def api_update_campaign_status_endpoint(campaign_id: str, req: StatusRequest):
